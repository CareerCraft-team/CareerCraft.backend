...
const User = require('../models/user_recruiter');
const verifyToken = require('../middlewares/verify-token');
const jwt = require('jsonwebtoken');
const upload = require('../middlewares/upload-photo');

router.post('/auth/recruiter/signup', upload.single('photo'), async(req, res) => {
    if (!req.body.email || !req.body.password) {
        ...
        try {
            ...
            res.json({
                success: true,
                token: token,
                message: 'Created a new recruiter!',
            });
        } catch (err) {
            res.status(500).json({
                success: false,
...
router.get('/auth/recruiter/user', verifyToken, async(req, res) => {
    try {
...
// Update profile
router.put('/auth/recruiter/user', verifyToken, async(req, res) => {
    try {
